    ...
    <div class="bg-gray-800 rounded-xl shadow-lg p-6">
      ...
      {#if isLoading}
        <div class="text-center py-4">
          <svg class="animate-spin h-8 w-8 text-purple-500 mx-auto" viewBox="0 0 24 24">
            <!-- Spinner SVG -->
          </svg>
        </div>
          ...
          {#each rooms as room}
            ...
            >
              <div class="flex justify-between items-start mb-2">
                <div>
                  <h3 class="text-purple-400 font-mono font-semibold">#{room.id}</h3>
                  <p class="text-gray-300 text-sm">Current letter: <span class="font-bold">{room.letter || '-'}</span></p>
                </div>
              ...
              <div class="flex justify-between text-sm">
                <div>
                  <p class="text-gray-400">
                    <span class="font-semibold text-purple-300">{room.columns?.length || 0}</span> categories
                  </p>
                  <p class="text-gray-400">
                    <span class="font-semibold text-purple-300">{room.playersWithAnswers?.size || 0}</span> players
                  </p>
                </div>
              </div>
            </div>
